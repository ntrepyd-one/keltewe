...
const CITIES: Record<string, [number, number]> = { 'la paz': [-16.50, -68.15], 'santa cruz': [-17.78, -63.18], 'cochabamba': [-17.39, -66.16], 'oruro': [-17.97, -67.11], 'potosi': [-19.58, -65.75], 'sucre': [-19.03, -65.26], 'tarija': [-21.53, -64.73], 'el alto': [-16.50, -68.19] };
const RISK = ['bloqueo','protesta','huelga','conflicto','incendio','inundacion','alerta','emergencia','accidente','paro','marcha','disturbio','mineria','entel','tigo','viva'];
...
export async function GET() {
  try {
    ...
    const articles = (data.articles || []).map((a: any) => { const combined = (a.title || '') + ' ' + (a.description || ''); return { id: a.url, title: a.title || '', description: (a.description || '').slice(0, 200), link: a.url || '', published: a.publishedAt || '', source: a.source?.name || 'NewsAPI', coords: geolocate(combined), risk_score: riskScore(combined), region: 'bolivia' }; });
